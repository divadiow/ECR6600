...
#define ECR_BLE_GATT_CHAR_MAX_NUM                                           (9)
...
typedef enum {      
    ECR_BLE_GATT_EVT_MTU_REQUEST = 0x10 ,               /**< MTU exchange request event, For Ble peripheral, we need to do reply*/
    ...
    ECR_BLE_GATT_EVT_NOTIFY_TX,                         /**< [Ble peripheral] Transfer data Callback, only report Result */
    ...
    ECR_BLE_GATT_EVT_READ_RX_SLAVE,                     /**< [Ble Peripheral] Get Char-Read Data */
...
typedef struct {
    uint8_t                         role;               /**< BLE role for this connection, see @ref ECR_BLE_ROLE_SERVER, or ECR_BLE_ROLE_CLIENT */
...
typedef struct {
    ...
    union {
        ...
        ECR_BLE_GATT_SVC_DISC_TYPE_T    svc_disc;       /**< Discovery All Service */
        ECR_BLE_GATT_CHAR_DISC_TYPE_T   char_disc;      /**< Discovery Specific Characteristic */
...
typedef enum
{
    ...
    ECR_BLE_IDX_WRITE_CHAR,
    ECR_BLE_IDX_WRITE_VAL,
    ECR_BLE_IDX_WRITE_IND_CFG,

    ECR_BLE_IDX_READ_CHAR,
    ECR_BLE_IDX_READ_VAL,
    ECR_BLE_IDX_READ_IND_CFG,

	ECR_BLE_IDX_NOTIFY_CHAR,
    ECR_BLE_IDX_NOTIFY_VAL,
    ECR_BLE_IDX_NOTIFY_IND_CFG,
...
typedef enum
{
	...
	/// Write Request
    PERM_WRITE_REQ_ENABLE	=(1<<2),
    PERM_WRITE_REQ_DISABLE	=(1<<3),
    
    /// Notification 
    PERM_NTF_ENABLE			=(1<<4),
    PERM_NTF_DISABLE		=(1<<5),
    
    /// Indication
    PERM_IND_ENABLE			=(1<<6),
    PERM_IND_DISABLE		=(1<<7),

	/// Write with no response
	PERM_WRITE_NO_RESPONSE	=(1<<8)
}ECR_ATTM_PERM_E;

typedef struct{
	uint8_t                     *value;              /**< Store handle value information */
	uint8_t                      len;				  /**< Store handle value len */
}ECR_BLE_HAND_VALUE_T;

typedef struct{
	uint8_t                     hand_name;              /**< Store handle information */
	ECR_BLE_HAND_VALUE_T        *hand_value;             /**< Store handle Value*/
}ECR_BLE_HAND_INFO_T;

typedef struct{
	uint8_t                  uuid[ATT_UUID_128_LEN];                      /**< 16 bit UUID  */          
	uint16_t                 perm;						/**< Attribute Permissions  */
}ECR_BLE_EACH_ATT_CFG_T;

typedef struct{
	ECR_BLE_EACH_ATT_CFG_T *each_att_cfg;
}ECR_BLE_ATT_CFG_T;

typedef struct{
	uint8_t                  uuid[ATT_UUID_128_LEN];
}ECR_BLE_PRM_SVC_UUID_T;
