...
char * strdup(const char *str)
{
	...
	{
		void *p = os_malloc_debug(size,filename,line);
		if (p)
			memset(p, 0, size);
		return p;
	}
...
#undef os_malloc
	...
	{
		return os_malloc_debug(size,__FILE__,__LINE__);
	}
#endif
#ifdef os_zalloc
#undef os_zalloc
	...
	{
		void *p = os_malloc_debug(size,__FILE__,__LINE__);
		if (p)
			memset(p, 0, size);
		return p;
	}
...
int system_irq_update(system_irq *cur_irq)
{
	...
	if (Find)
	{
		g_sys_irq[index].irq_time = cur_irq->irq_time;
		return 0;
	}
