...
static int utest_pwm_mutex_test(cmd_tbl_t *t, int argc, char *argv[])
{
	unsigned int ch = 2;
	unsigned int ch_mutex = 3;
	unsigned int ratio ;
	unsigned int fre;
	fre = (int)strtoul(argv[1], NULL, 0);
	ratio = (int)strtoul(argv[2], NULL, 0);
	chip_pwm_pinmux_cfg(PWM_CH2_USED_GPIO2);
	hal_pwm_init(ch);
	hal_pwm_config(ch, fre, ratio);
	drv_pwm_mutex_start(ch,ch_mutex);

	return 0;
}

CLI_SUBCMD(ut_pwm, pwm_test,utest_pwm_mutex_test, "utest_pwm1_test", "utest_pwm1_test");
