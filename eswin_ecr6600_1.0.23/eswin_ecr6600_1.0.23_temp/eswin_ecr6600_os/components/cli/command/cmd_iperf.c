...
static int iperf_stop_client(char *ip, int port)
{
    ...
    for (index = 0; index < IPERF_LINK_MAX_NUM; index++)
    {
        ...
        if (client->role == 'c' && client->server_port == port && !strcmp(client->server_hostname, ip)) {
            iperf_set_send_state(client, TEST_END);
            return CMD_RET_SUCCESS;
        }
    }
...
int iperf_sc_check(struct iperf_test *sc)
{
    ...
    for (index = 0; index < IPERF_LINK_MAX_NUM; index++)
    {
        ...
        if (sc->role == 's')
        {
            if (sc != iperf_thread[index] && sc->server_port == iperf_thread[index]->server_port)
            {
    ...
    for (index = 0; index < IPERF_LINK_MAX_NUM; index++)
    {
        if (iperf_thread[index] == NULL || iperf_thread[index]->freed == 1)
        {
            ...
            if (!iperf_sc_check(iperf_thread[index]))
            {
                ...
                iperf_thread[index]->taskid = os_task_create(taskname, LWIP_IPERF_TASK_PRIORITY, LWIP_IPERF_TASK_STACK_SIZE, iperf3_thread, iperf_thread[index]);
                printf("iperf(0x%x) %c port %d start\n",  iperf_thread[index]->taskid, iperf_thread[index]->role, iperf_thread[index]->server_port);
                return CMD_RET_SUCCESS;
            }
