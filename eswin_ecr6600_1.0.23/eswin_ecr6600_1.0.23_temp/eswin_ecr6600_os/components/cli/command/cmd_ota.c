...
int cmd_get_sn(cmd_tbl_t *h, int argc, char *argv[])
{
	...
	if((ret = develop_get_env_blob(OTA_SN, value, OTA_SN_LEN, NULL) ) == 0)
	{
...
{	
	...
	ret = develop_set_env_blob(OTA_SN, value, strlen(value));
...
{	
	...
	if((ret = develop_get_env_blob(OTA_PROJECT_ID, value, OTA_PROJECT_ID_LEN, NULL) )== 0)
	{
		os_printf(LM_CMD,LL_INFO, "NO %s!!\n", OTA_PROJECT_ID);
		return CMD_RET_FAILURE;
	}
...
{	
	...
	ret = develop_set_env_blob(OTA_PROJECT_ID, value, strlen(value));
