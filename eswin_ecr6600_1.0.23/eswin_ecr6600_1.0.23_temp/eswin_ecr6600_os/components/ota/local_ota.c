...
static int local_ota_message_send(ota_packet_type type, ota_action action)
{
    ...
    send_pkg.crc = ef_calc_crc8((unsigned char *)&send_pkg, sizeof(send_pkg) - sizeof(send_pkg.crc));
...
{
    unsigned char crc = ef_calc_crc8(buf, len - 1);
    ota_info_t *ota_info = get_local_ota_info();
    int parse_len = 0;
    char *ptr1, *ptr2;

    crc = ef_calc_crc8(buf, len - 1);
    if (crc != buf[len - 1])
    {
