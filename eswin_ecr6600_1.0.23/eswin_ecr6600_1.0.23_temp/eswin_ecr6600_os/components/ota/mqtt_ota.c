...
char MQTT_SERVICE_URL[32] = {0};
char MQTT_SERVICE_USERNAME[32] = {0};
char MQTT_SERVICE_PASSWD[32] = {0};
...
{
	if (develop_get_env_blob(OTA_PROJECT_ID, device_id, len, NULL) == 0) {
...
{
	if (develop_get_env_blob(OTA_SN, product_key, len, NULL) == 0) {
...
int mqtt_service_pub_errinfo_with_code(trs_mqtt_client_handle_t mq_client, int code, const char *errstr)
{
    ...
    char *data = os_malloc(DATA_MAX_LEN);
    char mac[6] = {0};
    char mac_string[32] = {0};
    char device_topic[TOPIC_MAX_LEN] = {0};
    ...
    snprintf(device_topic, sizeof(device_topic), "ESWIN/event/log/%s/%s/%s", MQTT_SERVICE_OEM, project_id, sn_key);
    ...
    return trs_mqtt_client_publish(mq_client, device_topic, data, strlen(data), 1, 0);
}
...
int mqtt_service_pub_deviceinfo_with_code(trs_mqtt_client_handle_t mq_client)
{
    ...
    wifi_get_mac_addr(0, (unsigned char *)mac);
    snprintf(mac_string, sizeof(mac_string), "%02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    snprintf(device_topic, sizeof(device_topic), "ESWIN/event/info/%s/%s/%s", MQTT_SERVICE_OEM, project_id, sn_key);
...
//	os_printf(LM_APP, LL_ERR, "OTA:set_data_str %s\n", set_data_str);
	...
	char *rtc_string = os_malloc(strlen((char*)set_data_str)+20);
...
static void service_ota_task(void *arg)
{
	...
	if ((develop_get_env_blob("mqtt_url", MQTT_SERVICE_URL, 32, NULL) == 0))
	{
		strcpy(MQTT_SERVICE_URL, MQTT_SERVICE_URL_DEFAULT);
		os_printf(LM_APP, LL_ERR, "OTA:No mqtt_url in dnv! default %s !\n", MQTT_SERVICE_URL);
	}
	else
	{
		os_printf(LM_APP, LL_ERR, "OTA:mqtt_url %s\n", MQTT_SERVICE_URL);
	}
	if (develop_get_env_blob("mqtt_user", MQTT_SERVICE_USERNAME, 32, NULL) == 0)
	{
		strcpy(MQTT_SERVICE_USERNAME, MQTT_SERVICE_USERNAME_DEFAULT);
		os_printf(LM_APP, LL_ERR, "OTA:No matt_user in dnv! default %s !\n", MQTT_SERVICE_USERNAME);
	}
	else
	{
		os_printf(LM_APP, LL_ERR, "OTA:mqtt_user %s\n", MQTT_SERVICE_USERNAME);
	}
	if (develop_get_env_blob("mqtt_password", MQTT_SERVICE_PASSWD, 32, NULL) == 0)
	{
		strcpy(MQTT_SERVICE_PASSWD, MQTT_SERVICE_PASSWD_DEFAULT);
		os_printf(LM_APP, LL_ERR, "OTA:No matt_password in dnv! default %s !\n", MQTT_SERVICE_PASSWD);
	}
	else
	{
...
{
	g_service_ota_task_handle = os_task_create("ota_service-task", 5, (4*1024), (task_entry_t)service_ota_task, NULL);
    return 0;
}
