...
#define AT_NET_TASK_STACK_SIZE   (4096)
...
int at_net_exit_pass_through(unsigned char *data, size_t len, int isr)
{
    // quxin net_conn_cfg_t   *cfg = get_net_conn_cfg();
    if (5 == len && data[0] == '+' && data[1] == '+'  && data[2] == '+' && data[3] == '\r' && data[4] == '\n') {
        at_net_abort_uart_rx();
...
void at_net_uart_rx_timeout(TimerHandle_t xTimer)
{
    ...
    if (at_net_exit_pass_through(send_db->buff, send_db->pos,0)) {
        return;
    }
...
void at_net_handle_data_from_target(void *param, const char *data, size_t size)
{
    ...
    if (send_db->pos >= send_db->threshold || send_db->hold_on) {
        os_printf(LM_APP, LL_INFO, "data overflow, busy %d:%d\n", send_db->pos, send_db->threshold);
        return;
    }
    ...
    key[0]='+';key[1]='+';key[2]='+';key[3]='\r';key[4]='\n';
    ...
    if(match == true)
    {
//        if (cfg->pass_through && at_net_exit_pass_through((unsigned char *)data, size, 1)) {
        if (cfg->pass_through && at_net_exit_pass_through(key, 5, 1)) {
            return;
        }            
    }


    if (0 == send_db->pos && cfg->pass_through) {
        xTimerResetFromISR(cfg->uart_rx_timeout, 0);
    }

    int step = 0;
    int flag = 0;
    for (i = 0; i < size && send_db->pos < send_db->threshold; ++i) {

        send_db->buff[send_db->pos] = data[i];

        if(cfg->use_end_flag == 1)
        {
            if(send_db->buff[send_db->pos] == '\\' && step == 0)
            {
                step = 1;
            }
            else if (send_db->buff[send_db->pos] == '\\' && step == 1)
            {
                step = 2;
                continue;
            }
            else if (send_db->buff[send_db->pos] == '0' && step == 1)
            {
                flag = 1;
                send_db->pos--;
                break;
            }
            else if (send_db->buff[send_db->pos] == '0' && step == 2)
            {
                step = 0;
            }
            else
            ...
            if(send_db->pos == send_db->threshold - 1 && data[i+1] == '0' && step == 1)
            {
                send_db->pos--;
            }
    ...
    if (send_db->pos >= send_db->threshold || flag == 1) {
        if (cfg->pass_through) {
            xTimerResetFromISR(cfg->uart_rx_timeout, 0);
        }
...
int at_net_client_tx()
{
    net_conn_cfg_t *cfg = get_net_conn_cfg();
    conn_send_buff_t *send_db = &cfg->send_buff;
    unsigned char *data = cfg->send_buff.buff;
    unsigned short out = 0, total = send_db->pos;
    struct sockaddr_in sock_addr;
    char result_str[24] = {0};
    int ret = 0;

    if (!send_db->client || !send_db->pos || conn_stat_connected != send_db->client->state) {
        os_printf(LM_APP, LL_INFO, "send error --%p:%d,%d\n", send_db->client, send_db->pos, send_db->client->state);
    ...
    if (conn_type_tcp == send_db->client->type) {
        ...
        while(out < total) {
            ret = sendto(send_db->client->fd, data + out, total - out, 0, (struct sockaddr *)&sock_addr, sizeof(sock_addr));
            if (ret < 0) {
                os_printf(LM_APP, LL_INFO, "send request failed.[ret:%d]..\n", ret);
                break;
            }
        ...
        while(out < total) {
            ret = trs_tls_conn_write(send_db->client->priv.tcp.tls, data + out, total - out);
            if (ret < 0) {
...
static void at_net_task(void *pvParams)
{
    ...
    while (1) {
        ...
        if (pdPASS == xQueueReceive(cfg->uart_rx_queue, &dummy, 0)) {
            ...
            //data handled, sendout it by socket.
            else
            {
                at_net_client_tx();
...
void at_net_task_init(dce_t* dce)
{
    ...
    cfg->uart_rx_timeout = xTimerCreate("uart_rx_timeout", pdMS_TO_TICKS(5000), pdTRUE, (void *)0, at_net_uart_rx_timeout);
...
int at_net_read_buff(conn_recv_data_t *recv_data, unsigned char *dst_buf, int length)
{
	...
	if (recv_data->write_pos - recv_data->read_pos + 1 >= act_len)
	{
		memcpy(dst_buf, &recv_data->buffer[recv_data->read_pos], act_len);
		recv_data->read_pos += (recv_data->read_pos == 0 ?(act_len - 1) : act_len);
	}
	else
	{
		int len1 = AT_MAX_RECV_DATA_LEN - recv_data->read_pos - 1;
		int len2 = act_len - len1;
		memcpy(dst_buf, &recv_data->buffer[recv_data->read_pos], len1);
		memcpy(dst_buf + len1, &recv_data->buffer[0], len2);
		recv_data->read_pos = len2 - 1;
	}	
