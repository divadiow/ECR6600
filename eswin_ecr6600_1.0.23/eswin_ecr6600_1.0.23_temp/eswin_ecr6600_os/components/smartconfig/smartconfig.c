...
static TaskHandle_t connect_wifi_handle = NULL;
...
{
    sc_log("sc_get_result sc_context ssid %s\n",sc_context.ssid);
    memcpy(result, &sc_context, sizeof(sc_result_t));
    sc_log("2 sc_get_result result ssid %s %d\n",result->ssid,sizeof(sc_result_t));
    return 0;
}
...
static int find_ap_in_aplist(const uint8_t *ap_mac, struct ap_info *t_ap)
{
    ...
    if (target_ap != NULL) {
        sc_log("target ap ssid %s channel %d\n", target_ap->ssid, target_ap->channel);
...
check:
    os_printf(LM_APP, LL_INFO, "type ----------- %d length %d\n",type,length );
    for(i = 0; i <= 4; i++)
    {
        os_printf(LM_APP, LL_INFO, "%d ",record_base[i]);
    }
...
/*        if((sc_context.data.magic_code.record[type][0] ^ 
            ...
            if(sc_context.total_len > 128) {
                ...
                sc_log("total_len:%d, AP is open mode !\n", sc_context.total_len);
...
static void multi_control_code(const uint8_t *frame)
{
    ...
    if (sc_memcmp(mutil_control_addr1, g_mutilAddr, 4) == 0) {
        ...
        controlFlag[0] = 1;
        sc_log("sc_context.total_len %d===sc_context.pswd_len=%d\n", sc_context.total_len, sc_context.pswd_len);
        ...
        controlFlag[1] = 1;
        sc_log("sc_context.ssid_crc %x===total_crc.=%x===\n", sc_context.ssid_crc, total_or);
        if ((zconfig_aplist[g_ssidIndex].ssidCrc != sc_context.ssid_crc) && g_ssidIndex > 0) {
            memset(sc_context.ssid, 0, ZC_MAX_SSID_LEN);
            for (num = 0; num < zconfig_aplist_num; num++) {
                if (zconfig_aplist[num].ssidCrc == sc_context.ssid_crc) {
...
static void multi_data_code(const uint8_t *frame)
{
    ...
    if (dataCount > numOfGroup) {
        ...
        if (dataCrc == sc_context.usr_data[sc_context.total_len - 1]) {
            ...
            sc_log("sc_context.pwd[ %s ]==sc_context.random_num=%x===\n", sc_context.pwd, sc_context.random_num);
            dataCount = 0;
            oldDataIndex = 0;
            sc_context.sc_state = SC_STATE_COMPLETE;
        }
...
static int process_null(void *packet, int size)
{
    ...
    {
        os_timer_stop(sc_cf.channel_timer);
        sc_log("stop sc_cf.channel_timer success.");
        msg = SC_STATUS_LOCK_CHANNEL;
        g_round = 0;
        os_queue_send(g_event_queue, (char *)&msg, sizeof(msg), portMAX_DELAY);
    }
        ...
        if (os_queue_receive(g_event_queue, (char *)&event, sizeof(event), (portTickType)portMAX_DELAY) == 0) {
            switch (event)
            {
                ...
                case SC_STATUS_TIMEOUT:
                    os_printf(LM_APP, LL_INFO, "SC_STATUS_TIMEOUT\n");
                    ...
                    goto stop;
                case SC_STATUS_STOP:
                    os_printf(LM_APP, LL_INFO, "SC_STATUS_STOP\n");
                    sc_context.sc_state = SC_STATE_STOPED;
                    wifi_set_promiscuous(false);
                    memset(&sc_head, 0, sizeof(sc_head_t));
                    resest_sc_data();
...
void sc_channel_switch_func(void* param)
{
    ...
    sc_log("sc set channel %d===g_round==%d==\n", chn, g_round);
    g_round++;
    g_current_channel = chn;
    sc_change_channel();
    wifi_rf_set_channel(chn);
    if(chn == 1 || chn == 6 || chn == 11){
        os_timer_changeperiod(sc_cf.channel_timer, 100);
    } else {
        os_timer_changeperiod(sc_cf.channel_timer, 100);
    }
}
...
int smartconfig_start(sc_callback_t cb)
{
    if (sc_context.sc_state == SC_STATE_STOPED) {
        ...
        sc_task_handle = (TaskHandle_t)os_task_create((const char *)"smart_config_task", 7, 512 * sizeof(StackType_t), (task_entry_t)sc_task, NULL);
...
TimerHandle_t connect_wifi_timer;
...
void connect_wifi_task(void *param) 
{
    ...
    while (1) {
        ...
        if (sta_ip != 0) {
            ...
            sc_log("===============connect_wifi_timeout======!\n");
            break;
        }
        os_msleep(100);
    }
    sc_log("===============connect_wifi_task vTaskDelete====== !\n");
    connect_wifi_handle = NULL;
    os_task_delete(0);
}
...
void sc_callback(smartconfig_status_t status, void *pdata)
{
    ...
    if (status == SC_STATUS_LOCK_CHANNEL){
        ...
        sc_log("target AP is in channel %d\n",lock_chn);
    }
    else if(status == SC_STATUS_GOT_SSID_PSWD) {
        sc_log("got ssid and password\n");
        ...
        connect_wifi_handle = (TaskHandle_t)os_task_create((const char *)"connect_wifi_task", 5, 512 * sizeof(StackType_t),
            (task_entry_t)connect_wifi_task, NULL);
        //if it's modifed to create this task during initialization later, it's priority should preferably be less than or equal to 5
        sc_log("====zyp===connect_wifi_handle=%d===\n", connect_wifi_handle);
        /* optional operation end */
    } else {
        sc_log("smartconfig status %d\n",status);
    }
}
