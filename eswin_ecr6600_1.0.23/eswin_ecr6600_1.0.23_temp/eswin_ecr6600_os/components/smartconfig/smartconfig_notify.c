...
static void sc_notify_send_task(void *pvParameters)
{
    ...
    if (g_sc_notify_send) {
        ...
        if (send_sock < 0) {
            os_printf(LM_WIFI, LL_ERR, "Creat udp socket failed\n");
            goto _end;
        }
        optval = 1;
    ...
    while (g_sc_notify_send) {
        ...
        os_msleep(500);
        os_printf(LM_WIFI, LL_INFO, "%02x %02x %02x %02x %02x %02x %02x\n", msg->random,
        ...
        if (sendlen > 0) {
            ...
            if (err == ENOMEM || err == EAGAIN) {
                os_printf(LM_WIFI, LL_ERR, "send failed, errno %d\n", err);
                continue;
            }
            os_printf(LM_WIFI, LL_ERR, "send notify sucess, errno %d\n", err);
            goto _end;
        }
    }
    os_msleep(100);
