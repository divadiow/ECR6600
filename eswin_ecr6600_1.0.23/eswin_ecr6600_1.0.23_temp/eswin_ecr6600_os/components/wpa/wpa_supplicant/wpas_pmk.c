...
void wpas_pmk_info_save(char *ssid, char *bssid, char *pwd, char *pmk, char *pmkid)
{
    ...
    wpa_printf(MSG_DEBUG, "ssid:%s pwd:%s bssid:%02x:%02x:%02x:%02x:%02x:%02x\r\n",ssid, pwd, bssid[0],bssid[1],bssid[2],bssid[3],bssid[4],bssid[5]);
    #if 0
    int i = 0;
    cli_printf("pmk:");
    for(i = 0; i < 32; i++)
...
int wpas_pmk_info_refresh(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    if(ssid->ssid == NULL || ssid->passphrase == NULL)
        return -1;
    wpa_printf(MSG_DEBUG, "wpas_pmk_info.ssid=%s pwd=%s pmkid_set:%d pmk_set:%d key_mgmt=%x\r\n",wpas_pmk_info.ssid, wpas_pmk_info.pwd, wpas_pmk_info.pmkid_set, wpas_pmk_info.pmk_set, wpas_pmk_info.key_mgmt);
    wpa_printf(MSG_DEBUG, "ssid->ssid=%s ssid->passphrase=%s key_mgmt=%x\r\n",ssid->ssid, ssid->passphrase, wpa_s->key_mgmt);
    ...
    ssid->psk_set = 1;
    wpa_printf(MSG_DEBUG, "ssid->psk_set = %d\r\n", ssid->psk_set);
#if defined (CONFIG_NV)
    if(set)
    {
...
{
    wpa_printf(MSG_DEBUG, "get pmkid[%d]\r\n", wpas_pmk_info.pmkid_set);
    /*int i;
    for(i = 0; i < 16; i++)
