...
#if !defined CONFIG_NATIVE_WINDOWS && !defined CONFIG_ECR6600
...
#endif /* CONFIG_ELOOP_KQUEUE */
		} else { //no timeout, use 2s timeout, by this, select don't block
			timeout = 1;
			_tv.tv_sec = 2;
			_tv.tv_usec = 0;
		}
...
#endif /* CONFIG_ELOOP_POLL */
	  	os_printf(LM_APP, LL_INFO, "eloop_run select again 0 \n");
#ifdef CONFIG_ELOOP_SELECT
		eloop_sock_table_set_fds(&eloop.readers, rfds);
		eloop_sock_table_set_fds(&eloop.writers, wfds);
		eloop_sock_table_set_fds(&eloop.exceptions, efds);
		res = lwip_select(eloop.max_sock + 1, rfds, wfds, efds,
			     timeout ? &_tv : NULL);
#endif /* CONFIG_ELOOP_SELECT */
	os_printf(LM_APP, LL_INFO, "eloop_run select again 1 \n");
