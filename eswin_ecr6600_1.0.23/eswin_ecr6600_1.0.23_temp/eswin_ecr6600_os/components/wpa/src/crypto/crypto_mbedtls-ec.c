...
#define USE_HARD_ECC					1
#define USE_HARD_ECC_DEBUG_MUL		0
#define USE_HARD_ECC_DEBUG_ADD		0
#define USE_HARD_ECC_DEBUG_SQR		0
#define USE_HARD_ECC_DEBUG_INV		0
#define USE_HARD_ECC_DEBUG_VER		0

#if USE_HARD_ECC
#include "ecc.h"

//void cli_hexdump(const void * addr, unsigned int size);
extern void cli_hexdump_int(const void * addr, unsigned int size);
...
#if USE_HARD_ECC
#if USE_HARD_ECC_DEBUG_ADD

	mbedtls_mpi one;
	mbedtls_ecp_point ress;

	mbedtls_mpi_init(&ress.X);
	mbedtls_mpi_init(&ress.Y);
	mbedtls_mpi_init(&ress.Z);

	mbedtls_mpi_init(&one);

	MBEDTLS_MPI_CHK(mbedtls_mpi_lset( &one, 1 ));
	MBEDTLS_MPI_CHK(mbedtls_ecp_muladd(&e->group, (mbedtls_ecp_point *)&ress, &one, (const mbedtls_ecp_point *)a , &one, (const mbedtls_ecp_point *)b));
#endif

	mbedtls_mpi_lset( &((mbedtls_ecp_point *)c)->Z, 1 );
	mbedtls_mpi_grow(&((mbedtls_ecp_point *)c)->X, 8);
	mbedtls_mpi_grow(&((mbedtls_ecp_point *)c)->Y, 8);

	ret = drv_ecc256_point_add((unsigned int *)((mbedtls_ecp_point *)c)->X.p, (unsigned int *)((mbedtls_ecp_point *)c)->Y.p, 
						(unsigned int *)((mbedtls_ecp_point *)a)->X.p, (unsigned int *)((mbedtls_ecp_point *)a)->Y.p,
						(unsigned int *)((mbedtls_ecp_point *)b)->X.p, (unsigned int *)((mbedtls_ecp_point *)b)->Y.p);

#if USE_HARD_ECC_DEBUG_ADD
	if (mbedtls_mpi_cmp_mpi(&ress.X, &((mbedtls_ecp_point *)c)->X))
	{
		cli_printf("wangc: %s  Q(x) is wrong\r\n", __func__);
		cli_hexdump_int(((const mbedtls_ecp_point *)c)->X.p, 32);
		cli_printf("wangc: %s  Q(x) should be: \r\n", __func__);
		cli_hexdump_int(ress.X.p, 32);
	}
	else
	{
		cli_printf("wangc: %s  Q(x) is ok\r\n", __func__);
	}

	if (mbedtls_mpi_cmp_mpi(&ress.Y, &((mbedtls_ecp_point *)c)->Y))
	{
		cli_printf("wangc: %s  Q(y) is wrong\r\n", __func__);
		cli_hexdump_int(((const mbedtls_ecp_point *)c)->Y.p, 32);
		cli_printf("wangc: %s  Q(y) should be: \r\n", __func__);
		cli_hexdump_int(ress.Y.p, 32);
	}
	else
	{
		cli_printf("wangc: %s  Q(y) is ok\r\n", __func__);
	}

	if (mbedtls_mpi_cmp_mpi(&ress.Z, &((mbedtls_ecp_point *)c)->Z))
	{
		cli_printf("wangc: %s  Q(z) is wrong\r\n", __func__);
		cli_hexdump_int(((const mbedtls_ecp_point *)c)->Z.p, 32);
		cli_printf("wangc: %s  Q(z) should be: \r\n", __func__);
		cli_hexdump_int(ress.Z.p, 32);
	}
	else
	{
		cli_printf("wangc: %s  Q(z) is ok\r\n", __func__);
	}
		...
		struct crypto_ec_point *res)
{
	...
	cli_printf("wangc: %s  P(x) \r\n", __func__);
	cli_hexdump_int(((const mbedtls_ecp_point *)p)->X.p, 32);
	cli_printf("wangc: %s  P(y) \r\n", __func__);
	cli_hexdump_int(((const mbedtls_ecp_point *)p)->Y.p, 32);

	cli_printf("wangc: %s  K \r\n", __func__);
	cli_hexdump_int(((const mbedtls_mpi *)b)->p, 32);

	cli_printf("wangc: %s  p \r\n", __func__);
	cli_hexdump_int(e->group.P.p, 32);
#endif

	mbedtls_mpi_lset( &((mbedtls_ecp_point *)res)->Z, 1 );
	mbedtls_mpi_grow(&((mbedtls_ecp_point *)res)->X, 8);
	mbedtls_mpi_grow(&((mbedtls_ecp_point *)res)->Y, 8);

	ret = drv_ecc256_point_mult((unsigned int *)((mbedtls_ecp_point *)res)->X.p, (unsigned int *)((mbedtls_ecp_point *)res)->Y.p, 
						(unsigned int *)((mbedtls_ecp_point *)p)->X.p, (unsigned int *)((mbedtls_ecp_point *)p)->Y.p, 
						(unsigned int *)((const mbedtls_mpi *)b)->p);

#if USE_HARD_ECC_DEBUG_MUL
	if (mbedtls_mpi_cmp_mpi(&ress.X, &((mbedtls_ecp_point *)res)->X))
	{
		cli_printf("wangc: %s  Q(x) is wrong\r\n", __func__);
		cli_hexdump_int(((const mbedtls_ecp_point *)res)->X.p, 32);
		cli_printf("wangc: %s  Q(x) should be: \r\n", __func__);
		cli_hexdump_int(ress.X.p, 32);
	}
	else
	...
	if (mbedtls_mpi_cmp_mpi(&ress.Y, &((mbedtls_ecp_point *)res)->Y))
	{
		cli_printf("wangc: %s  Q(y) is wrong\r\n", __func__);
		cli_hexdump_int(((const mbedtls_ecp_point *)res)->Y.p, 32);
		cli_printf("wangc: %s  Q(y) should be: \r\n", __func__);
		cli_hexdump_int(ress.Y.p, 32);
	}
	else
	...
	if (mbedtls_mpi_cmp_mpi(&ress.Z, &((mbedtls_ecp_point *)res)->Z))
	{
		cli_printf("wangc: %s  Q(z) is wrong\r\n", __func__);
		cli_hexdump_int(((const mbedtls_ecp_point *)res)->Z.p, 32);
		cli_printf("wangc: %s  Q(z) should be: \r\n", __func__);
		cli_hexdump_int(ress.Z.p, 32);
	}
	else
...
{
	int ret;
	unsigned long long data_zero[4] = {0x0ull, 0x0ull, 0x0ull, 0x0ull};
	...
	mbedtls_mpi_init(&ress.Y);
	mbedtls_mpi_init(&ress.Z);
	mbedtls_mpi_grow(&ress.X, 8);
	mbedtls_mpi_grow(&ress.Y, 8);
	mbedtls_mpi_grow(&ress.Z, 8);

	cli_printf("wangc: %s  x should be: \r\n", __func__);
	cli_hexdump_int(((mbedtls_ecp_point *)p)->Y.p, 32);

	ecp_opp(&e->group, &ress, (mbedtls_ecp_point *) p);

	cli_printf("wangc: %s  -x should be: \r\n", __func__);
	cli_hexdump_int(ress.Y.p, 32);

	mbedtls_mpi_free(&ress.X);
	mbedtls_mpi_free(&ress.Y);
	mbedtls_mpi_free(&ress.Z);	
#endif

	ret = drv_ecc256_mod_sub((unsigned int *)((mbedtls_ecp_point *)p)->Y.p, (unsigned int *)data_zero, 
							(unsigned int *)((mbedtls_ecp_point *)p)->Y.p);
...
#if USE_HARD_ECC
#if USE_HARD_ECC_DEBUG_SQR

struct crypto_bignum *crypto_ec_point_compute_y_sqr_soft(struct crypto_ec *e,
		...
		const struct crypto_bignum *x)
{
	...
	cli_printf("wangc: %s entry, dump x\r\n", __func__);
	cli_hexdump_int(((const mbedtls_mpi *)x)->p, 32);

	cli_printf("wangc: %s entry, dump mod\r\n", __func__);
	cli_hexdump_int(e->group.P.p, 32);

	crypto_ec_point_compute_y_sqr_soft(e, x);
#endif

	mbedtls_mpi_init(&num);
	mbedtls_mpi_init(y_sqr);

	MBEDTLS_MPI_CHK(mbedtls_mpi_lset(y_sqr, 1));
	MBEDTLS_MPI_CHK(mbedtls_mpi_grow(y_sqr, 8));
	MBEDTLS_MPI_CHK(mbedtls_mpi_grow(( mbedtls_mpi *)x, 8));
	MBEDTLS_MPI_CHK(mbedtls_mpi_grow(&num, 8));

	/* y^2 = x^3 + ax + b  mod  P*/

	/* Calculate x^2 mod  P*/
	MBEDTLS_MPI_CHK(drv_ecc256_mod_mult((unsigned int *)y_sqr->p, (unsigned int *)((const mbedtls_mpi *)x)->p,
		(unsigned int *)((const mbedtls_mpi *)x)->p));

#if USE_HARD_ECC_DEBUG_SQR
	cli_printf("wangc: %s entry, dump x^2\r\n", __func__);
	cli_hexdump_int(y_sqr->p, 32);

	MBEDTLS_MPI_CHK(mbedtls_mpi_lset(&num, 2));
	MBEDTLS_MPI_CHK(mbedtls_mpi_exp_mod(&temp, (const mbedtls_mpi *) x, &num, &e->group.P, NULL));
	cli_hexdump_int(temp.p, 32);
#endif

	/* Calculate x^2 + a mod  P*/
	MBEDTLS_MPI_CHK(mbedtls_mpi_lset(&num, 3));
	MBEDTLS_MPI_CHK(drv_ecc256_mod_sub((unsigned int *)y_sqr->p, (unsigned int *)y_sqr->p, 
		(unsigned int *)(num.p)));

#if USE_HARD_ECC_DEBUG_SQR
	cli_printf("wangc: %s entry, dump x^2 + a\r\n", __func__);
	cli_hexdump_int(y_sqr->p, 32);

	MBEDTLS_MPI_CHK(mbedtls_mpi_sub_mpi(&temp, &temp, &num));
	MBEDTLS_MPI_CHK(mbedtls_mpi_mod_mpi(&temp, &temp, &e->group.P));

	cli_hexdump_int(temp.p, 32);
#endif

	/* Calculate x^3 + ax mod  P*/
	MBEDTLS_MPI_CHK(drv_ecc256_mod_mult((unsigned int *)y_sqr->p, (unsigned int *)y_sqr->p, 
		(unsigned int *)((const mbedtls_mpi *)x)->p));

#if USE_HARD_ECC_DEBUG_SQR
	cli_printf("wangc: %s entry, dump x^3 + ax\r\n", __func__);
	cli_hexdump_int(y_sqr->p, 32);

	MBEDTLS_MPI_CHK(mbedtls_mpi_mul_mpi(&temp, (const mbedtls_mpi *) x, &temp));
	MBEDTLS_MPI_CHK(mbedtls_mpi_mod_mpi(&temp, &temp, &e->group.P));
	...
	/* Calculate x^3 + ax + b mod  P*/
	MBEDTLS_MPI_CHK(drv_ecc256_mod_add((unsigned int *)y_sqr->p, (unsigned int *)y_sqr->p,
		(unsigned int *)e->group.B.p));

#if USE_HARD_ECC_DEBUG_SQR
	cli_printf("wangc: %s entry, dump x^3 + ax + b\r\n", __func__);
	cli_hexdump_int(y_sqr->p, 32);

	MBEDTLS_MPI_CHK(mbedtls_mpi_add_mpi(&temp, &temp, &e->group.B));
	MBEDTLS_MPI_CHK(mbedtls_mpi_mod_mpi(&temp, &temp, &e->group.P));
		...
		const struct crypto_ec_point *p)
{
	...
	mbedtls_mpi_grow(&(( mbedtls_ecp_point *)p)->X, 8);
	mbedtls_mpi_grow(&(( mbedtls_ecp_point *)p)->Y, 8);

	result = drv_ecc256_point_verify((unsigned int *)((const mbedtls_ecp_point *)p)->X.p, 
									(unsigned int *)((const mbedtls_ecp_point *)p)->Y.p);

#if USE_HARD_ECC_DEBUG_VER
...
cleanup:
	...
	os_free(y_sqr_rhs);
	if ((ret == 0) && (on_curve == 1)) 
		cli_printf("wangc: %s entry, on curve\r\n", __func__);
