...
{
	os_memcpy(addr, net_if_get_mac_addr(fhost_get_netif_by_idx(l2->idx)), ETH_ALEN);
	return 0;
}
...
int l2_packet_send_by_net_if(net_if_t *net_if, const u8 *dst_addr, u16 proto,
		   const u8 *buf, size_t len)
{
	net_l2_send(net_if, buf, len, proto, (void *)dst_addr);
