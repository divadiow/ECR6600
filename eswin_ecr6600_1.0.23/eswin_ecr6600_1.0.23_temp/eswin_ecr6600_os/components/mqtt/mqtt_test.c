...
#define CLIENT_ID_STRING_MAX_LEN 23//defined in Section 1.5.3
...
int mqtt_sub(cmd_tbl_t *t, int argc, char *argv[])
{
    ...
    if ((topic == NULL) || (topic_len <= 0) || (topic_len >= PRINT_STRING_MAX_LEN))
    {
...
int mqtt_pub(cmd_tbl_t *t, int argc, char *argv[])
{
    ...
    {
        os_printf(LM_APP, LL_INFO, "topic or data error\n");
        return CMD_RET_FAILURE;
    }
    ...
    if ((topic_len >= PRINT_STRING_MAX_LEN) || (data_len >= PRINT_STRING_MAX_LEN))
    {
        os_printf(LM_APP, LL_INFO, "topic or data is too long\r\n");
    }
