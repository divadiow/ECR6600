...
{
    struct netif *nif = get_netif_by_index(vif);

    if(!nif){
        SYS_LOGV("get vif error \n");
        return;
    }
...
void wifi_set_opmode(wifi_work_mode_e opmode)
{
    ...
    wifi_netif_control(STATION_IF, sta_enable);
    wifi_netif_control(SOFTAP_IF, ap_enable);
...
sys_err_t wifi_load_ap_nv_info(wifi_ap_config_t *softap_info)
{
    if (hal_system_get_config(CUSTOMER_NV_WIFI_AP_SSID, softap_info->ssid , sizeof(softap_info->ssid) - 1) == 0 ||
        ...
        hal_system_get_config(CUSTOMER_NV_WIFI_MAX_CON, &softap_info->max_connect, sizeof(softap_info->max_connect)) == 0 ||
        hal_system_get_config(CUSTOMER_NV_WIFI_HIDDEN_SSID, &softap_info->hidden_ssid, sizeof(softap_info->hidden_ssid)) == 0) {
        return SYS_ERR;
    }
...
sys_err_t wifi_save_ap_nv_info(wifi_ap_config_t *softap_info)
{
    ...
    if (ret) {
        ...
        hal_system_del_config(CUSTOMER_NV_WIFI_STA_PMK);
        return SYS_ERR;
    }
...
sys_err_t wifi_load_nv_info(wifi_nv_info_t *nv_info)
{
    ...
    if (0 >= hal_system_get_config(CUSTOMER_NV_WIFI_STA_SSID, nv_info->sta_ssid, sizeof(nv_info->sta_ssid) - 1) ||
        ...
        0 >= hal_system_get_config(CUSTOMER_NV_WIFI_STA_PMK, nv_info->sta_pmk, sizeof(nv_info->sta_pmk))) {
    ...
    SYS_LOGE("load wifi nv:ssid[%s],pwd[%s],channel[%d],pmk[%02x%02x%02x],auto[%d]",
        nv_info->sta_ssid, nv_info->sta_pwd, nv_info->channel, nv_info->sta_pmk[0],
        nv_info->sta_pmk[1], nv_info->sta_pmk[2], nv_info->auto_conn);
    return SYS_OK;
...
sys_err_t wifi_save_nv_info(wifi_nv_info_t *nv_info)
{
    ...
    ret |= hal_system_set_config(CUSTOMER_NV_WIFI_STA_PMK, nv_info->sta_pmk, sizeof(nv_info->sta_pmk));
    ret |= hal_system_set_config(CUSTOMER_NV_WIFI_AUTO_CONN, &nv_info->auto_conn, sizeof(nv_info->auto_conn));

    SYS_LOGE("%d, %d, %d, %d,", strlen(nv_info->sta_ssid), strlen(nv_info->sta_pwd), sizeof(nv_info->channel), sizeof(nv_info->sta_pmk));
    SYS_LOGE("save wifi nv:ssid[%s],pwd[%s],channel[%d],pmk[%02x%02x%02x],auto[%d].",
        nv_info->sta_ssid, nv_info->sta_pwd, nv_info->channel, nv_info->sta_pmk[0],
        nv_info->sta_pmk[1], nv_info->sta_pmk[2], nv_info->auto_conn);
...
int wifi_update_nv_sta_connected(char *ssid, uint8_t channel)
{
    ...
    SYS_LOGE("wifi_update_nv_sta_connected()!!!, ssid: %s, nv_info->sta_ssid: %s", ssid, nv_info->sta_ssid);

    if (strcmp(ssid, nv_info->sta_ssid)) {
        SYS_LOGE("update wifi nv, but ssid mismatch!!!");
        SYS_LOGE("ssid: %s, nv_info->sta_ssid: %s", ssid, nv_info->sta_ssid);
        return SYS_ERR;
    }

    //ESWIN REMOVE for auto connect don't need save config bssid
