...
#define CONFIG_WIFI_CONN_CONNET_TIMEOUT 20
...
static void wifi_conn_task(void *arg)
{
    ...
    if (wifi_conn_scan() == 0) {
        if (wifi_conn_set_sta() == 0) {
            ...
            r_event = xEventGroupWaitBits(os_event_handle, OS_EVENT_STA_GOT_IP | OS_EVENT_STA_REMOVE_NETWORK, pdTRUE, pdFALSE, 20000);
