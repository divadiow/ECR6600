...
char timezone = 0; // GMT+08:00, -12,-11...-1,0,1...11,12
...
int set_system_time(unsigned int newtime)
{
    ...
    unix_time += timezone * ONE_HOUR2SEC;
    if (unix_time < 0)
    {
        return SNTP_RET_ERR;
    }

    newtime += timezone * ONE_HOUR2SEC;
    hal_rtc_set_time(newtime);
    os_printf(LM_APP, LL_INFO, "set time ok!timezone=%d,newtime=%d\r\n", timezone, newtime);
...
int set_timezone(char tz)
{
    ...
    if (timezone < 0)
    {
        if ((unsigned int)(cur_time - timezone * ONE_HOUR2SEC) < cur_time)
        {
    ...
    cur_time -= timezone * ONE_HOUR2SEC;

    if (tz > 0)
    {
        if ((unsigned int)(cur_time + tz * ONE_HOUR2SEC) < cur_time)
        {
    ...
    cur_time += tz * ONE_HOUR2SEC;
    hal_rtc_set_time(cur_time);
    irq = system_irq_save();
    timezone = tz;
    system_irq_restore(irq);
    snprintf(nv_str, sizeof(nv_str), "%d", tz);
...
void sntp_load_nv(void)
{
   ...
   // ret = customer_get_env_blob(NV_SNTP_TIMEZONE, time_zone_nv, sizeof(time_zone_nv), NULL);
	ret = hal_system_get_config(CUSTOMER_NV_SNTP_TIMEZONE,time_zone_nv,sizeof(time_zone_nv));
    if (ret <= 0)
    {
        timezone = 0;
		os_printf(LM_APP, LL_INFO, "time zone NV read err, resotre default value = %d!\n",timezone);
    }
    else
    {
        timezone = atoi(time_zone_nv);
        if ((timezone < -TIME_ZONE_MAX) || (timezone > TIME_ZONE_MAX))
        {
			timezone = 0;
            os_printf(LM_APP, LL_INFO, "time zone in NV is err, resotre default value =%d!\n",timezone);       
        }
    }

    //ret = customer_get_env_blob(NV_SNTP_UPDATEPERIOD, period_nv, sizeof(period_nv), NULL);
    ...
    //ret = customer_get_env_blob(NV_SNTP_SERVER, server[0], SNTP_SERVER_DNS_NAME_MAX_LEN, NULL);
