...
static os_sem_handle_t i2s_process;
...
int i2s_init_cfg(i2s_cfg_dev *i2s_cfg_dev)
{
	...
	T_I2S_REG_MAP * p_i2s_reg = (T_I2S_REG_MAP * )I2S_MEM_BASE;
	...
	tx_value = p_i2s_tx_reg->tx_cfg;
	rx_value = p_i2s_rx_reg->rx_cfg;
	...
	p_i2s_tx_reg->tx_cfg = tx_value;
	p_i2s_rx_reg->rx_cfg = rx_value;
	...
	{
		p_i2s_tx_reg->tx_fifo_cfg = 0;
		p_i2s_rx_reg->rx_fifo_cfg = 0;
	}
	else if( (i2s_cfg_dev->trans_mode == COMPRESSED_STEREO_MODE)&&(i2s_cfg_dev->compre_bit_width == COMPRE_BIT_8_WIDTH) )
	{
		p_i2s_tx_reg->tx_fifo_cfg = 2;
		p_i2s_rx_reg->rx_fifo_cfg = 2;
	}
	...
	{
		p_i2s_rx_reg->rx_fifo_cfg = 4;
	}
	
	/* i2s tx rx limit  */
	p_i2s_tx_reg->tx_limit = 1;
	p_i2s_rx_reg->rx_limit = 1;

	/*i2s fenpin 8fenpin----0, 4fenpin-----1  */
	p_i2s_tx_reg->tx_cfg = ((p_i2s_tx_reg->tx_cfg) & 0xFFFFFFF7)|0x8;
	p_i2s_rx_reg->rx_cfg = ((p_i2s_rx_reg->rx_cfg) & 0xFFFFFFF7)|0x8;



	i2s_process = os_sem_create(1, 0);

	if(i2s_cfg_dev->i2s_dma == 1)
	{
		i2s_dma_chan=drv_dma_ch_alloc();
		if (i2s_dma_chan < 0)
		{
			return -1;
		}
		drv_dma_isr_register(i2s_dma_chan, i2s_dma_callback_isr, NULL);
	}
	else
	{
		#if 0
...
void i2s_master_write(unsigned char * bufptr, unsigned int length)
{
	...
	if(i2s_cfg.i2s_dma==0)
	{
		for (index = 0; index < length; index++)
		{
			...
			do 
			{
				fifo = (p_i2s_tx_reg->tx_level) & 0x1F;
			} while (fifo > 14);
	...
	else
	{
		...
		i2s_tx_cfg.dst = (unsigned int)&(p_i2s_tx_reg->tx_fifo);
		...
		drv_dma_cfg(i2s_dma_chan,  &i2s_tx_cfg);
		drv_dma_start(i2s_dma_chan);
		os_sem_wait(i2s_process, I2S_WAIT_FOREVER);
	}
...
void i2s_master_read(unsigned   char *bufptr, unsigned int length)
{
	...
	if(i2s_cfg.i2s_dma == 1)
	{
		...
		i2s_rx_cfg.src = (unsigned int)&p_i2s_rx_reg->rx_fifo;
		...
		drv_dma_cfg(i2s_dma_chan,  &i2s_rx_cfg);
		drv_dma_start(i2s_dma_chan);
    ...
    while (cnt < length / 4)
    {
        while(((p_i2s_rx_reg->rx_level) & 0x1F) == 0);
        *(pbuf + cnt) = p_i2s_rx_reg->rx_fifo;
        cnt++;
    }
	}
	//arch_irq_restore(flags);
...
void i2s_master_read_loop(void)
{
    ...
    while (1)
    {
        if(((p_i2s_rx_reg->rx_level) & 0x1F) > 0)
        {
            p_i2s_tx_reg->tx_fifo = p_i2s_rx_reg->rx_fifo;
        }
    }
}
