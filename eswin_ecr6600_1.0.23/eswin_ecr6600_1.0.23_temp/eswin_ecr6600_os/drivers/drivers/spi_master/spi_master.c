...
spi_interface_config_t spi_master;
...
int drv_spi_master_close()
{
	if(spi_tx_process)
	{
	    os_sem_destroy(spi_tx_process);
	}
	
	if(spi_rx_process)
	{
	    os_sem_destroy(spi_rx_process);
	}
...
//	}
	...
	chip_clk_disable(CLK_SPI1_APB);
	chip_clk_disable(CLK_SPI1_SEL);
	return 0;
}


int spi_config_at(spi_interface_config_t *spi_master_dev)
{
	unsigned int value;
	spi_reg * p_spi_reg = (spi_reg * )MEM_BASE_SPI1;
	
	/*addr length*/
	if((spi_master.addr_pha_enable & BIT(29)) && (spi_master_dev->addr_len))
	{
		value = p_spi_reg ->transFmt;
		value = (value & (~SPI_FORMAT_ADDRLEN_FOUR_BYTE)) | ((spi_master_dev->addr_len-1) << 16);
		p_spi_reg ->transFmt = value;
	}
	
	/*addr enable*/
	spi_master.addr_pha_enable    = spi_master_dev->addr_pha_enable;
	
	/*cmd enable & trans mode*/
	spi_master.cmd_write = spi_master_dev->cmd_write;
	spi_master.cmd_read = spi_master_dev->cmd_read;
...
int spi_master_write(unsigned char *bufptr,spi_transaction_t* trans_desc )
{
	...
	if(spi_dma_enable == 0)
	{
		...
		while(trans_desc->length)
		{
		    offset = MIN(512, trans_desc->length);
			num_temp = offset;

			memcpy(p_spi_dev->spi_tx_buf, bufptr + write_data, offset);
			...
			{
				p_spi_reg->transCtrl = spi_master.addr_pha_enable | spi_master.dummy_bit | spi_master.cmd_write | SPI_TRANSCTRL_WCNT((offset)-1);
			}
...
int spi_master_write_read(unsigned char*write_buf, unsigned char* read_buf , unsigned int write_length, unsigned int read_length )
{
	if((write_length) >512 || (write_length) <0 )
	{
		os_printf(LM_OS, LL_INFO, "Tansfer length Error\n");
		return -1;
	}
	
	unsigned int * rxBuf = (unsigned int *)(read_buf);
	unsigned long flags = system_irq_save();	
	spi_reg * p_spi_reg = (spi_reg * )MEM_BASE_SPI1;
	unsigned int left,data;
	unsigned int rx_num,i = 0;
	{
		memcpy(p_spi_dev->spi_tx_buf, write_buf, write_length);
		spi_master_bus_ready();
		//clear rx & tx fifo
		if(spi_clear_fifo()) 
		{
		...
		{
			p_spi_reg->transCtrl = spi_master.addr_pha_enable | spi_master.dummy_bit | spi_master.cmd_write | SPI_TRANSCTRL_WCNT((write_length)-1)| SPI_TRANSCTRL_RCNT((read_length) - 1);
		}
		p_spi_reg->cmd = 0x00;
		...
		{
			data = 0;	
			left = min((write_length), 4);
			for (i = 0; i < left; i++) 
			{
				data |= write_buf[i] << (i * 8);	
			}
			SPI_WAIT_TX_READY(p_spi_reg->status); 	
			p_spi_reg->data = data;
			write_length -=left;	
