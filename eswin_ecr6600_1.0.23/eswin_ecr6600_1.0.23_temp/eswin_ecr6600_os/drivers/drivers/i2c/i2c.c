...
static os_sem_handle_t i2c_process;
unsigned char *dma_i2c_src=(unsigned char *)0xA0000;
unsigned char *dma_i2c_dst=(unsigned char *)0xA1000;
unsigned int i2c_dma_chan=0;
unsigned int i2c_dma_flag =0;

os_queue_handle_t queue_handle_i2c;
unsigned int tx_remain_length = 0;
...
{
	T_I2C_REG_MAP * p_i2c_reg = (T_I2C_REG_MAP * )MEM_BASE_I2C;
	return  ((p_i2c_reg->Status) & I2C_FLAG);
}
...
int  i2c_init_cfg(I2C_InitTypeDef* I2C_InitStruct)
{
	...
	{
		i2c_dma_chan=drv_dma_ch_alloc();
		if (i2c_dma_chan < 0)
		{
			return -1;
		}
		drv_dma_isr_register(i2c_dma_chan, i2c_dma_callback_isr, NULL);
...
int i2c_master_read(unsigned int slave_addr,unsigned char *bufptr, unsigned int len,unsigned int cmd_len)
{
	...
	T_I2C_REG_MAP * p_i2c_reg = (T_I2C_REG_MAP * )MEM_BASE_I2C;
	p_i2c_reg->Addr = slave_addr;
	unsigned int i; 
	if(p_i2c_reg->Status & BIT(11))
		return -1;
	...
	else
	{
		...
		if(cmd_len)
		{
			...
			i2c_tx_cfg.dst=(unsigned int)&p_i2c_reg->Data;
			i2c_tx_cfg.len=cmd_len;
			i2c_tx_cfg.mode=DMA_CHN_I2C;
			drv_dma_cfg(i2c_dma_chan,  &i2c_tx_cfg);
			drv_dma_start(i2c_dma_chan);
			I2C_TransConfig(I2C_Direction_Transmitter,1); 
			I2C_Cmd(I2C_CMD_IssueDataTrans);
			os_sem_wait(i2c_process, I2C_WAIT_FOREVER);
			os_msdelay(1);
		}
		drv_dma_status_clean(i2c_dma_chan);
		i2c_rx_cfg.src=(unsigned int)&p_i2c_reg->Data;
		i2c_rx_cfg.dst=(unsigned int)(dma_i2c_dst+cmd_len);
		i2c_rx_cfg.len=len;
		i2c_rx_cfg.mode=DMA_CHN_I2C;
		drv_dma_cfg(i2c_dma_chan,  &i2c_rx_cfg);
		drv_dma_start(i2c_dma_chan);
		I2C_TransConfig(I2C_Direction_Receiver, len);
		I2C_Cmd(I2C_CMD_IssueDataTrans);
		os_sem_wait(i2c_process, I2C_WAIT_FOREVER);
	...
	if(i2c_master.I2C_DMA == I2C_DMA_Disable)
	{
		...
		while(len)
		{
			if(len <= 0xFF)
			{
				...
				I2C_Cmd(I2C_CMD_IssueDataTrans);
				tx_remain_length = len;
				tx_length = len;
				tx_depth = DRV_I2C_FIFO_DEPTH;
				while(tx_depth)
				{
					tx_depth--;
					p_i2c_reg->Data = bufptr[i++];
					tx_remain_length--;
					if(tx_remain_length == 0)
					{
						break;
				...
				if(tx_remain_length)
				{
					memcpy(dma_i2c_src, &bufptr[i], tx_remain_length);
					p_i2c_reg->IntEn = I2C_FLAG_FIFOEmpty;
					os_sem_wait(i2c_process, I2C_WAIT_FOREVER);
				}
			}
			...
			{
				I2C_TransConfig(I2C_Direction_Transmitter, 0xFF); 
				I2C_Cmd(I2C_CMD_IssueDataTrans);
				tx_remain_length = 0xFF;
				tx_length = 0xFF;
				if(tx_remain_length)
				{
					memcpy(dma_i2c_src, &bufptr[i], tx_remain_length);
