...
{	
	...
	if(((sec_cnt % 2) == 0)
		...
		&& psm_is_stat_sleep()
        && beacon_cnt_old != 0 
        && beacon_cnt_old == psm_dbg_infs.psm_dbg_wifi.cnt_rec_beacon)
    ...
    extern int g_dhcp_wait_time;
    if(g_dhcp_wait_time > 0)
    {
    	os_printf(LM_OS, LL_INFO, "%s wait_time:%d\n",__func__,g_dhcp_wait_time);
        g_dhcp_wait_time --;
    }
#endif
