...
//static T_HASH_REG_MAP * HASH_REG_BASE = (T_HASH_REG_MAP *)MEM_BASE_HASH;
...
@param[in] is256:When the input parameter is 1, the initial value of SHA256 HASH is selected,
           otherwise it is SHA512.        
*/
...
static void drv_sha256_internal_done(const unsigned char *input, unsigned int ilen, unsigned int total, unsigned char output[32])
{
	...
	else
	{
		...
		HASH_REG_BASE->Cfg = HASH_CFG_LAST | HASH_CFG_SHA256;   ///< Write the configuration to the HASH_LP_CFG register
		HASH_REG_BASE->PcrLen[0] = total;  ///< Write the message length into the HASH_LP_PCR_LEN register
		HASH_REG_BASE->PcrLen[1] = 0;

		for (i = 0, j = 0; i < ilen; i += 4, j += 1)
		{
			GET_UINT32_BE(data, input, i);
			HASH_REG_BASE->MdIn[j] = data;   ///< Write data to HASH_LP_MDIN register
		}
...
int drv_hash_sha512_ret(const unsigned char *input, unsigned int ilen, unsigned char output[64])
{
	...
	while (1)
	{
		unsigned int len, i, j, data;

		if (ilen <= 128)
		{
			len = ilen;
			HASH_REG_BASE->Cfg = HASH_CFG_LAST | HASH_CFG_SHA512;   ///< Write the configuration to the HASH_LP_CFG register
			HASH_REG_BASE->PcrLen[0] = total;   ///< Write the message length into the HASH_LP_PCR_LEN register
			HASH_REG_BASE->PcrLen[1] = 0;
		}
		else
		...
		for (i = 0, j = 0; i < len; i += 4,j += 1)
		{
			GET_UINT32_BE(data, input + offset, i);
			HASH_REG_BASE->MdIn[j] = data;   ///< Write data to HASH_LP_MDIN register
		}
