...
static void spiFlash_QE_cfg_restore(T_SPI_FLASH_DEV *p_flash_dev)
{
	...
	else if(p_flash_dev->flash_param.flash_id == 0x16701C)	//EN25QH32B
	{
		//spiFlash_format_wr_fast(SPIFLASH_CMD_WRSR_2,QE | SPIFLASH_STATUS_2_QE,2);
	}
	else
...
int drv_spiflash_erase(unsigned int addr,  unsigned int len)
{
	...
	*/
	...
	for (i=0; i<eraseSectorCnt; i++)
	{
		...
		{
			spiFlash_cmd_wrEnable();
			if(flash_dev.flash_param.flash_id == EN25QH32B || flash_dev.flash_param.flash_id == EN25Q16B )
			{
					break;
			}
			else
		...
		if((data & SPIFLASH_STATUS_WEL) == 0 && ((flash_dev.flash_param.flash_id != EN25QH32B) || (flash_dev.flash_param.flash_id == EN25Q16B)))
		{
...
int drv_spiflash_OTP_Read(int addr, int length, unsigned char *pdata)
{
	...
	if(p_flash_dev->flash_param.flash_id == EN25QH32B || p_flash_dev->flash_param.flash_id == EN25Q16B )
	{
